...
def run():
    ...
    model = Sequential()
    model.add(Dense(input_dim=784, output_dim=150, init='glorot_normal'))
    model.add(Activation('sigmoid'))
    model.add(Dense(input_dim=150, output_dim=10, init='glorot_normal'))
    model.add(Activation('sigmoid'))

    model.set_loss(loss='categorical_crossentropy')
    model.set_optimizer(optimizer=SGD(lr=0.5, momentum=0.5, decay=1e-6))
